...
function Test-PasswordRequirements {
    ...
    if ($Password.Length -lt 12 -or $Password.Length -gt 123) {
        Write-Host "Error: Password must be 12-123 characters long" -ForegroundColor Red
        return $false
    }
...
Write-Host "- Must be 12-123 characters long"
Write-Host "- Must contain uppercase, lowercase, numbers, and special characters"
Write-Host ""
do {
    ...
    if (-not (Test-PasswordRequirements -Password $vmPasswordText)) {
        Write-Host ""
        continue
    }
...
Write-Host ""
Write-Host "Creating storage account for boot diagnostics..."
$storageAccountName = "sa" + ($resourceGroupName -replace '-','').ToLower() + (Get-Date).ToString("HHmmss")
...
$null = az vm boot-diagnostics enable `
    --resource-group $resourceGroupName `
    --name $vmName `
